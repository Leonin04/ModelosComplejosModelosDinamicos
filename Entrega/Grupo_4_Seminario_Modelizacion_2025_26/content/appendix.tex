...
En este apéndice se presenta el código fuente desarrollado en Python para la calibración de los modelos. Se han utilizado las librerías \texttt{PyMC} para la inferencia Bayesiana y \texttt{SciPy} para el ajuste no lineal de la ley de potencia.

\subsection{Script de Calibración y Visualización}

A continuación se detalla el algoritmo completo utilizado para generar los resultados de la Sección \ref{sec:analisis_empirico}.

\begin{lstlisting}[language=Python, caption=Código Completo de Calibración GARCH y Difusión, label=list:python_code]
import pandas as pd
import numpy as np
